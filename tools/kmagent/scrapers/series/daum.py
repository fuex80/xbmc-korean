...
class SeriesFetcher:
    ...
    def ParseEpisodePageList(self,id):
        ...
        for i in range(self.last_epnum-1):
            ...
            self.meta.EpisodeInfo[(self.Season,epnum)] = (u"제%d회"%epnum, "", "", "")

    def ParseEpisodePageByUrl(self,url):
        #print url
        resp = urllib.urlopen(url);
    ...
    def GetEpisodeInfoByContent(self,soup,url):
        for item in soup.findAll('li',{'id' : re.compile("^itemId_")}):
            ...
            if epnum == 0:      # non-numbered item
                epnum = self.last_epnum-1
                url = ""
            if epnum <= 0: break
            if epnum > self.max_epnum:
                self.max_epnum = epnum
