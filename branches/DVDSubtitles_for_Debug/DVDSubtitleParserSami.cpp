...
bool CDVDSubtitleParserSami::Open(CDVDStreamInfo &hints)
{
	...
	{
		CLog::Log(LOGERROR, "CDVDSubtitleParserSami::Fail to initialize regular express");
		return false;
	}
	...
	{
		CLog::Log(LOGERROR, "CDVDSubtitleParserSami::Fail to open subtitle");
		return false;
	}
	...
	{
		CLog::Log(LOGDEBUG, "CDVDSubtitleParserSami::Current tags [%lld] Read [%s]", currentTag, line);
		...
		if (check_style(currentTag))
		{
			...
			if (line[pos2] == '.')
			{
				...
				languageName.ToLower();
				// will implement later...
				// vecLanguages.push_back(languageName);

				CLog::Log(LOGDEBUG, "CDVDSubtitleParserSami::Get a new language [%s]", languageName.c_str());
		...
		while ((pos=tags.RegFind(strUTF8.c_str(), pos)) > -1)
		{
			...
			if (fullTag == "<!--")
			{
				CLog::Log(LOGDEBUG, "CDVDSubtitleParserSami::Start comment");
			...
			{
				CLog::Log(LOGDEBUG, "CDVDSubtitleParserSami::End comment");
			...
			else if (tagNames.RegFind(fullTag.c_str()) > -1)
			{
				...
				if (tagName.Find("sami") != -1)
				{
					if (tagName[0] == '/')
					{
						CLog::Log(LOGDEBUG, "CDVDSubtitleParserSami::Get a </sami> tag");
				...
				else if (tagName.Find("head") != -1)
				{
					if (tagName[0] == '/')
					{
						CLog::Log(LOGDEBUG, "CDVDSubtitleParserSami::Get a </head> tag");
				...
				else if (tagName == "style" || tagName == "/style")
				{
					if (tagName[0] == '/')
					{
						CLog::Log(LOGDEBUG, "CDVDSubtitleParserSami::Get a </style> tag");
					...
					else
					{
						while ((pos2 = tagOptions.RegFind(fullTag.c_str(), pos2)) > -1)
						{
							...
							if (tagOptionName == "type")
							{
								...
								CLog::Log(LOGDEBUG, "CDVDSubtitleParserSami::End <style> type is [%s]", tagOptionValue.c_str());
				...
				else if (tagName.Find("body") != -1)
				{
					if (tagName[0] == '/')
					{
						CLog::Log(LOGDEBUG, "CDVDSubtitleParserSami::Get a </body> tag");
				...
				else if (tagName == "b" || tagName == "/b")
				{
					if (tagName[0] == '/')
					{
						CLog::Log(LOGDEBUG, "CDVDSubtitleParserSami::Get a </b> tag");
				...
				else if (tagName == "font" || tagName == "/font")
				{
					...
					{
						CLog::Log(LOGDEBUG, "CDVDSubtitleParserSami::Get a </font> tag");
					...
					else
					{
						...
						while ((pos2 = tagOptions.RegFind(fullTag.c_str(), pos2)) > -1)
						{
							...
							if (tagOptionName == "color")
							{
								...
								CLog::Log(LOGDEBUG, "CDVDSubtitleParserSami::Get a <font> tag color is [%s/%s]", tagOptionValue.c_str(), tempColorTag.c_str());
							}
							tagOptionName.clear();
							tagOptionValue.clear();
						}
				...
				else if (tagName == "i" || tagName == "/i")
				{
					if (tagName[0] == '/')
					{
						CLog::Log(LOGDEBUG, "CDVDSubtitleParserSami::Get a <i> tag");
					...
					while ((pos2 = tagOptions.RegFind(fullTag.c_str(), pos2)) > -1)
					{
						...
						if (tagOptionName == "class")
						{
							...
							*/
							...
							CLog::Log(LOGDEBUG, "CDVDSubtitleParserSami::Get a <p> tag class is [%s]", tagOptionValue.c_str());
						}
						tagOptionName.clear();
						tagOptionValue.clear();
					}
				...
				else if (tagName == "sync")
				{
					while ((pos2 = tagOptions.RegFind(fullTag.c_str(), pos2)) > -1)
					{
						...
						if (tagOptionName == "start")
						{
							...
							CLog::Log(LOGDEBUG, "CDVDSubtitleParserSami::Get a <sync> tag start time is %f", startTime);
						}
						tagOptionName.clear();
						tagOptionValue.clear();
					}
				...
				{
					CLog::Log(LOGDEBUG, "CDVDSubtitleParserSami::Get a <br> tag");
				...
				{
					CLog::Log(LOGDEBUG, "CDVDSubtitleParserSami::Unknown Tag [%s]", fullTag.c_str());
		...
		if (check_sami(currentTag) && check_body(currentTag) && !check_comment(currentTag) && startTime != -1)
		{
			if (pOverlay && pOverlay->iPTSStartTime != DVD_NOPTS_VALUE && startTime != pOverlay->iPTSStartTime)
			{
				...
				CLog::Log(LOGDEBUG, "CDVDSubtitleParserSami::Start new subtitle starttime is [%f]", startTime);
			...
			if (strUTF8.IsEmpty())
			{
				CLog::Log(LOGDEBUG, "CDVDSubtitleParserSami::Skip empty subtitle");
				continue;
			}
		...
		{
			CLog::Log(LOGDEBUG, "CDVDSubtitleParserSami::Skip subtitle [%s]", strUTF8.c_str());
		}
	}
...
CStdString CDVDSubtitleParserSami::GetColorCode(CStdString color_text)
{
	...
	CLog::Log(LOGDEBUG, "CDVDSubtitleParserSami::Fail to parse color text [%s]", color_text.c_str());
