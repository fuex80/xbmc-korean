...
class AudioFile(object):
    ...
    def AnalyzeMp3(self):
        ...
        while True:
            buf = self.f.read(3)
            if len(buf) < 3 or self.Tell() > 50000:
                # ID tag is not found
                self.f.seek(0,0)
                self.audioStart = 0
                return
    ...
    def AnalyzeOgg(self):
        ...
        while True:
            ...
            if len(buf) < 27 or self.Tell() > 50000:
                # parse error
                raise FormatError
            if buf[0:4] != 'OggS':
    ...
    def AnalyzeWma(self):
        ...
        while True:
            buf = self.f.read(16)
            if len(buf) < 16 or self.Tell() > 50000:
                raise FormatError
            guid = buf.encode("hex");
