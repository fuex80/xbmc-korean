...
class TedTalks:
    ...
    def getVideoDetails(self, url):
        ...
        #get id,intro from text
        (id,intro) = re.search(';ti=(\d+)&amp;introDuration=(\d+)&', html).group(1,2)
        return {'Title':title, 'Director':speaker, 'Genre':'TED', 'Plot':plot, 'PlotOutline':plot, 'id':id, 'url':url, 'introDuration':intro}
