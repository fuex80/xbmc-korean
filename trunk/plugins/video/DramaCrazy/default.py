...
def CATEGORIES():
        addDir('Chinese Drama','http://www.dramacrazy.net/v/chinese-drama/',1,'http://upload.wikimedia.org/wikipedia/commons/thumb/5/5b/Flag_of_Hong_Kong.svg/800px-Flag_of_Hong_Kong.svg.png')
        addDir('Chinese Movies','http://www.dramacrazy.net/v/chinese-movie/',2,'http://upload.wikimedia.org/wikipedia/commons/thumb/5/5b/Flag_of_Hong_Kong.svg/800px-Flag_of_Hong_Kong.svg.png')
        addDir('Japanese Drama','http://www.dramacrazy.net/v/japanese-drama/',1,'http://www.mapsofworld.com/images/world-countries-flags/japan-flag.gif')
        addDir('Japanese Live Action','http://www.dramacrazy.net/v/japanese-live-action/',2,'http://www.mapsofworld.com/images/world-countries-flags/japan-flag.gif')
        addDir('Japanese Movies','http://www.dramacrazy.net/v/japanese-movie/',2,'http://www.mapsofworld.com/images/world-countries-flags/japan-flag.gif')
        addDir('Korean Drama','http://www.dramacrazy.net/v/korean-drama/',1,'http://www.nitsd.com/Korean_Flag2.gif')
        addDir('Korean Movies','http://www.dramacrazy.net/v/korean-movie/',2,'http://www.nitsd.com/Korean_Flag2.gif')
        addDir('Taiwanese Drama','http://www.dramacrazy.net/v/taiwanese-drama/',1,'http://www.8asians.com/wp-content/uploads/2008/02/taiwan.gif')
        addDir('Taiwanese Movies','http://www.dramacrazy.net/v/taiwanese-movie/',2,'http://www.8asians.com/wp-content/uploads/2008/02/taiwan.gif')
...
def PAGES(url):
	...
	if  url == 'http://www.dramacrazy.net/v/chinese-drama/':
		match=re.compile('<a href="http://www.dramacrazy.net/v/chinese-drama/page/(.+?)/"').findall(link)
		xx = 1
		name = 'Chinese Dramas ' + str(int(xx))
		addDir(name,url,2,'')
		for pageitem in match:
			xx=xx+1
			name = 'Chinese Dramas ' + str(int(xx))
			addDir(name,'http://www.dramacrazy.net/v/chinese-drama/page/'+pageitem+'/',2,'')
			
	if  url == 'http://www.dramacrazy.net/v/japanese-drama/':
		match=re.compile('<a href="http://www.dramacrazy.net/v/japanese-drama/page/(.+?)/"').findall(link)
		yy = 1
		name = 'Japanese Dramas ' + str(int(yy))
		addDir(name,url,2,'')
		for pageitem in match:
			yy=yy+1
			name = 'Japanese Dramas ' + str(int(yy))
			addDir(name,'http://www.dramacrazy.net/v/japanese-drama/page/'+pageitem+'/',2,'')

	if  url == 'http://www.dramacrazy.net/v/korean-drama/':
		match=re.compile('<a href="http://www.dramacrazy.net/v/korean-drama/page/(.+?)/"').findall(link)
		zz = 1
		name = 'Korean Dramas ' + str(int(zz))
		addDir(name,url,2,'')
		for pageitem in match:
			zz=zz+1
			name = 'Korean Dramas ' + str(int(zz))
			addDir(name,'http://www.dramacrazy.net/v/korean-drama/page/'+pageitem+'/',2,'')


	if  url == 'http://www.dramacrazy.net/v/taiwanese-drama/':
		match=re.compile('<a href="http://www.dramacrazy.net/v/taiwanese-drama/page/(.+?)/"').findall(link)
		aa = 1
		name = 'Taiwanese Dramas ' + str(int(aa))
		addDir(name,url,2,'')
		for pageitem in match:
			aa=aa+1
			name = 'Taiwanese Dramas ' + str(int(aa))
			addDir(name,'http://www.dramacrazy.net/v/taiwanese-drama/page/'+pageitem+'/',2,'')
...
def INDEX(url):
	...
	link=response.read()
	response.close()
	match=re.compile('href="(.+?)" rel="bookmark">(.+?)</a>').findall(link)
...
def VIDEOLINKS(url,name):
        ...
        #try:
                ...
                #for a in match:
                        ...
                        #myspace=f.read()
                        #comp=re.compile('<a href="redirect?(.+?)"')
        ...
        #GOOGLE
        try:
                p=re.compile('<embed id="VideoPlayback" src="http://video.google.com/googleplayer.swf.+?docid=(.+?)&.+?"')
                match=p.findall(link)
                for a in match:
