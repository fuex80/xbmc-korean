...
#AnimeCrazy for XBMC by exiledx

def CATEGORIES():
        addDir('Anime Shows','http://www.animecrazy.net/category/completed-anime-shows/',1,'')
        addDir('Anime Movies','http://www.animecrazy.net/category/anime-movies/',2,'')
...
def PAGES(url):
	...
	if  url == 'http://www.animecrazy.net/category/completed-anime-shows/':
		match=re.compile('<a href="http://www.animecrazy.net/category/completed-anime-shows/page/(.+?)/"').findall(link)
		xx = 1
		name = 'Anime Shows ' + str(int(xx))
		addDir(name,url,2,'')
		for pageitem in match:
			xx=xx+1
			name = 'Anime Shows ' + str(int(xx))
			addDir(name,'http://www.animecrazy.net/category/completed-anime-shows/page/'+pageitem+'/',2,'')
...
def VIDEOLINKS(url,name):
        ...
        #try:
                ...
                #link=response.read()
                #response.close()
                #p=re.compile('<embed src="http://www.veoh.com/videodetails.+?permalinkId=(.+?)&.+?;id=.+?;player=videodetailsembedded.+?;videoAutoPlay=0.+?')
        ...
        try:
                ...
                link=response.read()
                response.close()
                p=re.compile('<param name="movie" value="http://www.veoh.com/videodetails.+?permalinkId=(.+?)&amp;id=.+?&amp;player=.+?&amp;videoAutoPlay=.+?>')
                match=p.findall(link)
                for code in match:
                        veoh='http://127.0.0.1:64653/'+match[0]+"?.avi"
                        addLink("Play "+name,veoh,"http://www.veoh.com/static/marketing/wallpapers/Veoh_Logo1680.jpg")
        ...
        try:
                ...
                match=p.findall(link)
                for a in match:
                        f=urllib2.urlopen("http://www.flashvideodownloader.org/download.php?u=http://www.dailymotion.com/swf/"+str(a))
                        myspace=f.read()
                        comp=re.compile('<a href="(.+?)" title="Click to Download"><font color=red>')
                        for url in comp.findall(myspace):
                                addLink ('Play '+name,url,'')
