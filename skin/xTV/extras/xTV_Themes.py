...
def DownloaderClass(url,file,name):
	...
	dp.create(xbmc.getLocalizedString(31100),xbmc.getLocalizedString(31104)," ",name)
	...
	try:
		...
		dp.close()
		xbmcgui.Dialog().ok(xbmc.getLocalizedString(31100),xbmc.getLocalizedString(31105)," ",xbmc.getLocalizedString(31108))
		xbmc.executebuiltin('XBMC.Notification('+name+','+xbmc.getLocalizedString(31101)+')')
		return 1
	except:
		dp.close()
		xbmcgui.Dialog().ok(xbmc.getLocalizedString(31100),xbmc.getLocalizedString(31107)," ",xbmc.getLocalizedString(31109))
		xbmc.executebuiltin('XBMC.Notification('+xbmc.getLocalizedString(31100)+','+xbmc.getLocalizedString(31106)+')')
		return 0
...
def main():
	try:
		...
		if not themename_list == []:
			themeid = xbmcgui.Dialog().select(xbmc.getLocalizedString(31100),themename_list)
			filesplit = os.path.splitext(filename_list[themeid])
			if themeid == -1:
				return
			elif os.path.exists("Q:\\skin\\"+currentskin+"\\media\\"+filesplit[0]+".xpr"):
				xbmcgui.Dialog().ok(xbmc.getLocalizedString(31100),xbmc.getLocalizedString(31102)," ",xbmc.getLocalizedString(31108))
				return
			else:
				DownloaderClass(path_list[themeid],filename_list[themeid],themename_list[themeid])
		else:
			xbmcgui.Dialog().ok(xbmc.getLocalizedString(31100),xbmc.getLocalizedString(31103))
	except:
		xbmcgui.Dialog().ok(xbmc.getLocalizedString(31100),xbmc.getLocalizedString(31110)," ",xbmc.getLocalizedString(31111))
