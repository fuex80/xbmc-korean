...
class UI:
    ...
    def playVideoWithSubtitle(self):
        video = TedTalks.getVideoDetails(self.main.args.url)
	smiList = []

        subtitle = Subtitle()
	jsonStr = subtitle.getTEDSubtitlesByTalkID( video['id'] )
	smiPriStr = subtitle.convertTEDSubtitlesToSMISubtitles( jsonStr, int(video['introDuration']) )

	smiFile = xbmc.translatePath("special://temp/ted-%s.%s.smi") % (video['id'], subtitle.language)
	smiList.append( smiFile )
	f=open(smiFile, 'w')
	f.write( smiPriStr.encode('utf-8') )
	f.close()

	smiEngStr = u""
        if __settings__.getSetting('genEngSubtitle') == 'true' and subtitle.language != 'eng':
	    subeng = Subtitle('eng')
	    jsonStr = subeng.getTEDSubtitlesByTalkID( video['id'] )
	    smiEngStr = subeng.convertTEDSubtitlesToSMISubtitles( jsonStr, int(video['introDuration']) )

	    smiFile = xbmc.translatePath("special://temp/ted-%s.eng.smi") % video['id']
	    smiList.insert(0, smiFile)
	    f=open(smiFile, 'w')
	    f.write( smiEngStr )
	    f.close()

	    smiFile = xbmc.translatePath("special://temp/ted-%s.merged.smi") % video['id']
	    smiList.insert(0, smiFile)
	    f=open(smiFile, 'w')
	    f.write( subtitle.mergeSMI( smiEngStr, smiPriStr ).encode('utf-8') )
	    f.close()
        ...
        import time
        time.sleep(5)   # temporal fix until setResolvedUrl bug get fixed
	for smi in smiList :
	    xbmc.Player().setSubtitles( smi )
...
class Main:
    ...
    def checkMode(self):
        ...
        if mode is None:
            ...
            if __settings__.getSetting('subtitleEnable') == 'true':
                UI().playVideoWithSubtitle()
            else:
                UI().playVideo()
        elif mode == 'newTalks':
            UI().newTalks()
